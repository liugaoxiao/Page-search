// ==UserScript==
// @name 页面关键字搜索
// @description 页面关键字搜索：高亮、结果列表、历史、多关键字、正则、快捷键、导出、同源 iframe 与可调磨砂玻璃 UI。
// @match *://*/*
// @run-at document-end
// @grant GM.log
...
(() => {
  ...
  let config = { ...defaultConfig, ...loadJson(STORAGE_KEY, {}) }
  ...
  const addStyle = () => {
    ...
    style.textContent = `
      ...
      #${ROOT_ID}.ps-topbar { top: 8px; left: 10px; right: 10px; }
      ...
      #${ROOT_ID} .ps-toggle {
        ...
        background: var(--ps-accent); color: #fff; font-size: 22px;
        box-shadow: 0 8px 24px rgba(15, 23, 42, .28);
      }
      #${ROOT_ID}.ps-topbar .ps-toggle { width: 100%; height: 42px; border-radius: 14px; }
      #${ROOT_ID} .ps-panel {
        display: none;
        width: min(390px, calc(100vw - 28px)); max-height: min(78vh, 680px); overflow: auto;
        padding: 12px; border: 1px solid rgba(148, 163, 184, .45); border-radius: 16px;
        background: var(--ps-panel-bg); box-shadow: 0 12px 32px rgba(15, 23, 42, .28);
      }
      #${ROOT_ID}.ps-glass .ps-panel { backdrop-filter: blur(var(--ps-blur)); -webkit-backdrop-filter: blur(var(--ps-blur)); }
      #${ROOT_ID}.ps-topbar .ps-panel { width: 100%; max-height: min(70vh, 620px); }
      #${ROOT_ID}.ps-open .ps-toggle { display: none; }
      #${ROOT_ID}.ps-open .ps-panel { display: block; }
      #${ROOT_ID} .ps-title { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; color: #0f172a; font-size: 15px; font-weight: 800; }
      #${ROOT_ID} .ps-close { width: 28px; height: 28px; border: 0; border-radius: 999px; background: rgba(241,245,249,.9); color: #334155; font-size: 17px; }
      #${ROOT_ID} .ps-tabs { display: grid; grid-template-columns: repeat(3,1fr); gap: 6px; padding: 3px; margin-bottom: 10px; border-radius: 12px; background: rgba(241,245,249,.82); }
      #${ROOT_ID} .ps-tab { height: 32px; border: 0; border-radius: 10px; background: transparent; color: #475569; font-weight: 800; }
      #${ROOT_ID} .ps-tab.ps-active { background: rgba(255,255,255,.92); color: var(--ps-accent); box-shadow: 0 1px 4px rgba(15,23,42,.08); }
      #${ROOT_ID} .ps-page { display: none; }
      #${ROOT_ID} .ps-page.ps-active { display: block; }
      #${ROOT_ID} .ps-row { display: flex; gap: 8px; margin-bottom: 8px; }
      #${ROOT_ID} input[type="search"], #${ROOT_ID} input[type="text"] {
        flex: 1; min-width: 0; height: 38px; padding: 0 10px; border: 1px solid #cbd5e1; border-radius: 10px;
        outline: none; background: rgba(255,255,255,.92); color: #0f172a; font-size: 15px;
      }
      #${ROOT_ID} input[type="search"]:focus, #${ROOT_ID} input[type="text"]:focus { border-color: var(--ps-accent); box-shadow: 0 0 0 3px color-mix(in srgb, var(--ps-accent) 16%, transparent); }
      #${ROOT_ID} .ps-search { height: 38px; padding: 0 12px; border: 0; border-radius: 10px; background: var(--ps-accent); color: white; font-size: 14px; font-weight: 800; }
      #${ROOT_ID} .ps-nav { flex: 1; min-width: 0; height: 36px; padding: 0 6px; border: 0; border-radius: 10px; background: color-mix(in srgb, var(--ps-accent) 11%, white); color: var(--ps-accent); font-size: 13px; font-weight: 800; }
      #${ROOT_ID} .ps-clear { background: rgba(248,250,252,.9); color: #475569; }
      #${ROOT_ID} .ps-status { min-height: 20px; margin: 2px 2px 8px; color: #64748b; font-size: 13px; }
      #${ROOT_ID} .ps-result-list, #${ROOT_ID} .ps-history-list { display: none; max-height: 230px; overflow: auto; margin-top: 8px; border: 1px solid #e2e8f0; border-radius: 12px; background: rgba(248,250,252,.8); }
      #${ROOT_ID} .ps-result-list.ps-visible, #${ROOT_ID} .ps-history-list.ps-visible { display: block; }
      #${ROOT_ID} .ps-result, #${ROOT_ID} .ps-history-item { width: 100%; display: block; padding: 9px 10px; border: 0; border-bottom: 1px solid #e2e8f0; background: transparent; color: #334155; text-align: left; line-height: 1.35; font-size: 13px; }
      #${ROOT_ID} .ps-result:last-child, #${ROOT_ID} .ps-history-item:last-child { border-bottom: 0; }
      #${ROOT_ID} .ps-result.ps-current { background: color-mix(in srgb, var(--ps-accent) 16%, white); color: #1e3a8a; }
      #${ROOT_ID} .ps-result-index { font-weight: 900; margin-right: 5px; color: var(--ps-accent); }
      #${ROOT_ID} .ps-setting { display: flex; gap: 10px; align-items: center; justify-content: space-between; padding: 10px 2px; border-bottom: 1px solid #e2e8f0; color: #0f172a; font-size: 14px; }
      ...
      #${ROOT_ID} input[type="checkbox"] { width: 20px; height: 20px; accent-color: var(--ps-accent); }
      #${ROOT_ID} input[type="color"] { width: 42px; height: 30px; border: 0; background: transparent; }
      #${ROOT_ID} input[type="range"] { width: 120px; accent-color: var(--ps-accent); }
      #${ROOT_ID} select { height: 34px; padding: 0 8px; border-radius: 9px; border: 1px solid #cbd5e1; background: rgba(255,255,255,.92); color: #0f172a; }
  ...
  const renderHistory = () => {
    const list = document.querySelector<HTMLElement>(`#${ROOT_ID} .ps-history-list`)
    if (!list) return
  ...
  const useSelection = () => {
    ...
    if (!selected) { setStatus("当前页面没有选中文字"); return }
  ...
  const exportResults = async () => {
    if (!matches.length) { setStatus("没有可导出的搜索结果"); return }
    const text = [`页面：${document.title}`, `网址：${location.href}`, `关键字：${keyword}`, `数量：${matches.length}`, "", ...matches.map((mark, index) => `${index + 1}. ${getSnippet(mark)}`)].join("\n")
    try {
      await navigator.clipboard.writeText(text)
      setStatus("搜索结果已复制到剪贴板")
    } catch {
      prompt("复制搜索结果", text)
    }
  }
  ...
  const renderSettings = () => {
    ...
    if (!page) return
    page.innerHTML = `
      <label class="ps-setting"><span>显示位置<small>悬浮底部、悬浮顶部或固定顶部搜索条</small></span><select class="ps-config-position"><option value="bottom" ${config.position === "bottom" ? "selected" : ""}>底部悬浮</option><option value="top" ${config.position === "top" ? "selected" : ""}>顶部悬浮</option><option value="topbar" ${config.position === "topbar" ? "selected" : ""}>顶部搜索条</option></select></label>
      ...
      <label class="ps-setting"><span>打开时使用选中文字</span><input class="ps-config-selection" type="checkbox" ${config.autoSelection ? "checked" : ""} /></label>
  ...
  const openPanel = (tab = "search") => {
    ...
    switchTab(tab)
    const selected = String(getSelection()?.toString() || "").trim().replace(/\s+/g, " ")
    if (tab === "search" && config.autoSelection && selected && input()?.value !== selected) {
  ...
  const createPanel = () => {
    ...
    el.innerHTML = `
      ...
      <div class="ps-panel">
        ...
        <div class="ps-page ps-active" data-page="search">
          <div class="ps-row"><input class="ps-input" type="search" placeholder="输入关键字；多关键字用逗号或双空格分隔" autocomplete="off" /><button class="ps-search" type="button">搜索</button></div>
          <div class="ps-status">输入关键字后点击搜索</div>
          <div class="ps-row"><button class="ps-nav ps-prev" type="button">上一个</button><button class="ps-nav ps-next" type="button">下一个</button><button class="ps-nav ps-selection" type="button">选中文字</button><button class="ps-nav ps-export" type="button">复制结果</button><button class="ps-nav ps-clear" type="button">清除</button></div>
    ...
    el.querySelector(".ps-toggle")?.addEventListener("click", () => openPanel("search"))
    el.querySelector(".ps-close")?.addEventListener("click", () => el.classList.remove("ps-open"))
    ...
    el.querySelector(".ps-selection")?.addEventListener("click", useSelection)
    el.querySelector(".ps-export")?.addEventListener("click", exportResults)
    ...
    el.querySelector(".ps-input")?.addEventListener("keydown", (event) => { if (event.key === "Enter") search(); if (event.key === "Escape") el.classList.remove("ps-open") })
